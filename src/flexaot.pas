...
begin
  ...
  WriteLn(StdErr, '  flexaot --bf=100 --hf=10 --bw=20 --h=50 --d=46 --fck=25 --fyk=500 --mk=100');
  WriteLn(StdErr, '  flexaot --bf=100 --hf=10 --bw=20 --h=50 --d=46 --fck=25 --fyk=500 --mk=100 --field=rho');
...
begin
  ...
  if bf <= 0 then
  begin
    WriteLn(StdErr, 'ERRO: bf must be positive (received: ', bf:0:1, ' cm)');
    Halt(EXIT_INVALID_RANGE);
  end;
  
  if hf <= 0 then
  begin
    WriteLn(StdErr, 'ERRO: hf must be positive (received: ', hf:0:1, ' cm)');
    Halt(EXIT_INVALID_RANGE);
  end;
...
begin
  ...
  begin
    verificacao := 'ERRO: Flange too thick - use rectangular section with width bf' + LineEnding +
                   'CURRENT: bf=' + FloatToStr(bf) + 'cm, hf=' + FloatToStr(hf) + 'cm, bw=' + FloatToStr(bw) + 'cm, h=' + FloatToStr(h) + 'cm, d=' + FloatToStr(d) + 'cm, Mk=' + FloatToStr(mk) + 'kN.m, fck=' + FloatToStr(fck_orig) + 'MPa' + LineEnding +
                   'SUGGESTION: Use rectangular section design (flexao) with bw=bf';
    As_trac := 0;
    As_comp := 0;
    dominio := 0;
    ami := 0;
  ...
  begin
    ...
    begin
      ...
      verificacao := 'ERRO: Section insufficient (Domain 4 - brittle failure)' + LineEnding +
                     'CURRENT: bf=' + FloatToStr(bf) + 'cm, hf=' + FloatToStr(hf) + 'cm, bw=' + FloatToStr(bw) + 'cm, h=' + FloatToStr(h) + 'cm, d=' + FloatToStr(d) + 'cm, Mk=' + FloatToStr(mk) + 'kN.m, fck=' + FloatToStr(fck_orig) + 'MPa' + LineEnding +
                     'SUGGESTION: Increase height to h >= ' + FloatToStr(h * 1.2) + 'cm OR increase flange width to bf >= ' + FloatToStr(bf * 1.2) + 'cm';
    end;
  end
  else
  begin
    ...
    begin
      verificacao := 'ERRO: Double reinforcement in domain 2 not allowed' + LineEnding +
                     'CURRENT: bf=' + FloatToStr(bf) + 'cm, hf=' + FloatToStr(hf) + 'cm, bw=' + FloatToStr(bw) + 'cm, h=' + FloatToStr(h) + 'cm, d=' + FloatToStr(d) + 'cm, Mk=' + FloatToStr(mk) + 'kN.m, fck=' + FloatToStr(fck_orig) + 'MPa' + LineEnding +
                     'SUGGESTION: Increase section dimensions (h, bf, or bw) to avoid domain 2 with double reinforcement';
      As_trac := 0;
      As_comp := 0;
      dominio := 0;
      Exit;
    ...
    begin
      verificacao := 'ERRO: Section insufficient - compression steel would be in tension' + LineEnding +
                     'CURRENT: bf=' + FloatToStr(bf) + 'cm, hf=' + FloatToStr(hf) + 'cm, bw=' + FloatToStr(bw) + 'cm, h=' + FloatToStr(h) + 'cm, d=' + FloatToStr(d) + 'cm, Mk=' + FloatToStr(mk) + 'kN.m, fck=' + FloatToStr(fck_orig) + 'MPa' + LineEnding +
                     'SUGGESTION: Increase section dimensions (h, bf, or bw)';
      As_trac := 0;
      As_comp := 0;
      dominio := 0;
      Exit;
